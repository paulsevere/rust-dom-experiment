...
fn update_dom(state: &StateRef) {
    ...
    fn only_active(todo: &Todo) -> bool {
        todo.completed == false
    }
    fn only_completed(todo: &Todo) -> bool {
        todo.completed == true
    }
    fn all(_: &Todo) -> bool {
        true
    ...
    let selected_anchor: Element = document()
        .query_selector(filter_anchor_selector.as_str())
        .unwrap()
        .try_into()
        .unwrap();
    ...
    for (index, todo) in state_borrow.todo_list.iter().enumerate().filter(
        |&(_, todo)| {
            filter(todo)
        },
    )
...
fn main() {
    ...
    let title_entry: InputElement = document()
